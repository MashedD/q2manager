...
static void SelectBrowserPath(App &app, const fs::path &path) {
    ...
    switch (app.browser.target) {
        ...
        case BrowserTarget::Autoexec: break;
        default: break;
    }
...
static std::string IndexedManagedName(const char *prefix, size_t index, const fs::path &source) {
...
static bool PrepareBaseq2Links(const App &app, const Preset &preset, fs::path &baseq2Dir, std::string &error) {
    ...
    const bool hasConfigs = !preset.configs.empty();
    ...
    if (hasConfigs) {
        ...
        for (const auto &cfg : preset.configs) {
            ...
            const std::string linkName = IndexedManagedName("q2m_cfg_", configIndex++, source);
            if (!RecreateSymlink(source, baseq2Dir / linkName, error)) return false;
            out << "exec " << CfgQuote(linkName) << '\n';
        }
